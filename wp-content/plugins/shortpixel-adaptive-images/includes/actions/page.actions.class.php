	...
	class Actions {
		...
		public static function handle() {
            ...
            if(!isset($_POST['spainonce']) || !wp_verify_nonce($_POST['spainonce'], 'shortpixel-ai-settings')) {
                wp_send_json( [ 'success' => false, 'message' => __( 'Invalid nonce.', 'shortpixel-adaptive-images' ) ] );
            }
			$page = $_POST[ 'page' ];
			$data = $_POST[ 'data' ];
						...
						if ( is_object( $category ) || is_array( $category ) ) {
							...
							foreach ( $category as $option => $value ) {
								...
								if ( $category_name === 'areas' && $option === 'parse_css_files' ) {
                                    ...
                                    if($value === true  && $current_options->areas->parse_css_files === false) {
                                    ...
                                    } else if($value === false && $current_options->areas->parse_css_files === true) {
