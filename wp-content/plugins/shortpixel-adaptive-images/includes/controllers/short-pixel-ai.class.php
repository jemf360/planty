...
class ShortPixelAI {
    ...
    public function init_ob() {
        if ($this->isWelcome()) {
            ...
            if ( $this->settings->areas->parse_css_files && $integrations->has( 'wp-optimize', 'enable_css' ) ) {
...
//        }
        $this->varyCacheSupport = !$this->options->settings->compression->webp_detect
                               || (strpos($this->options->settings->behaviour->api_url, \ShortPixelAI::DEFAULT_API_AI) !== false);
    }
	...
	public function toolbar_top_menu($wp_admin_bar) {
		...
		if (   self::userCan( 'manage_options' )
			...
			if ( Options::_()->settings_areas_parseCssFiles && !!Options::_()->pages_onBoarding_hasBeenPassed) {
				$wp_admin_bar->add_menu( array(
    ...
    public function register_js($name, $file, $enqueue = true, $onlyMin = false, $addVersion = true, $deps = []) {
        $this->_register('js', $name, $file, $onlyMin, $addVersion, $deps);
        if($enqueue) {
            wp_enqueue_script($name);
        }
    }

    protected function _register($type, $name, $file, $onlyMin = true, $addVersion = false, $deps = []) {
	    ...
	    $version = $addVersion
            ...
            : null;
	    if($type === 'js') {
            wp_register_script( $name, $url, $deps, $version, true);
        } else {
