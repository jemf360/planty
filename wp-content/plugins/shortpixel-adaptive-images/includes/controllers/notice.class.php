...
//				}
		...
		public function renderNotices() {
			...
			*/
			...
			if ( !isset( $dismissed->wprocketcss ) && $this->ctrl->settings->areas->parse_css_files
            ...
            if ( !isset( $dismissed->temporary_redirect )) {
                ...
                if($domainStatus && ($domainStatus->TemporaryRedirectOrigin === true || $domainStatus->TemporaryRedirectOrigin === "true")) {
            ...
            if ( $integrations->has('swift-performance') && !!$this->ctrl->settings->areas->parse_css_files && !isset( $dismissed->swift_performance ) ) {
			...
			if ( $domain_status && $domain_status->Status !== 2 && $domain_status->Status !== -3 ) {
				...
				if ( $domain_status->Status === 1 ) {
					...
					$messages = [
                        ...
                        . ' <div id="spaiProposeUpgradeShade" data-ajaxnonce="' . wp_create_nonce( 'ajax_request' )
